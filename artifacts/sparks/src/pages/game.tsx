...
export default function Game() {
  ...
  // Host auto-advances to results screen when both answers are submitted.
  // Answers preserved in gameState for the per-round reveal in results.tsx.
  // Ready-based round continuation (results → next question) is handled in results.tsx.
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [hasBothAnswers, isHost, currentRoom?.gameState.phase]);
  ...
  // Skip: host-triggered, advances card without recording answers
        ...
        <div className="flex justify-between items-center mb-4 glass-panel rounded-full px-4 py-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={skipsRemaining <= 0 ? 'No skips left' : `Skip (${skipsRemaining} left)`}
            >
              {skipsRemaining <= 0 ? 'No skips left' : `Skip (${skipsRemaining})`}
            </button>
